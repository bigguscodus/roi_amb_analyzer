...
def hla_parser(region:str,data='hla.dat') -> dict:
    '''This function takes imgt file and returns a dictionary {region:{exon_number:sequence}'''
    total_exon_names_seqs = dict()  # лист для хранение отображений последовательностей на экзоны
    hla = SeqIO.parse(data, 'imgt')
    for allele in hla:
        if allele.description.split(',')[0].startswith(region) and len(
            ...
            total_exon_names_seqs[allele.description.split(',')[0]] = {
                ...
                allele.features if f.type == 'exon'}
